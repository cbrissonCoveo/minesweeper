        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                if !self.open_fields.contains(&pos){
                    if self.lost && self.mines.contains(&pos) && !self.flagged_fields.contains(&pos) {
                        f.write_str("💣 ")?;
...
impl Minesweeper {
        ...
        Minesweeper {
            ...
            mines: {
                ...
                while mines.len() < mine_count {
                    mines.insert((random_range(0, width), 
    ...
    pub fn open(&mut self, pos: Position) -> Option<OpenResult> {
        if self.open_fields.contains(&pos) {
            let mine_count = self.neighboring_mines(pos);
            let flag_count = self.iter_neighbors(pos).filter(|&neighbor| {
                self.flagged_fields.contains(&neighbor)
            }).count() as u8;
